...
/* USACE process-fit: their phases on top, EGON capability cards beneath */
.phase-bar{display:grid;grid-template-columns:repeat(auto-fit,minmax(88px,1fr));gap:3px;
  margin:16px 0 0;}
.ph{background:#E4E8EC;color:var(--navy);padding:9px 5px;text-align:center;font-size:9.5px;
  font-weight:700;letter-spacing:1px;text-transform:uppercase;}
.ph .pn{display:block;font-family:'Oswald',sans-serif;font-size:13px;letter-spacing:1px;}
.ph.done{background:var(--blue);color:#fff;}
.ph.now{background:var(--red);color:#fff;}
.gates{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:3px;
  margin:3px 0 18px;}
.gate{background:var(--cream);border-top:3px solid var(--gold);padding:9px 10px;font-size:10px;
  font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:var(--navy);
  text-align:center;}
.fit-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(255px,1fr));gap:18px;
  margin:18px 0;}
.fit{border:1px solid #ccc;border-top:4px solid var(--red);padding:15px 18px 18px;}
.fit.pending{border-top-color:var(--gold);}
.fit .phn{font-family:'Oswald',sans-serif;font-size:12px;letter-spacing:2px;color:var(--red);}
.fit .fh{font-weight:700;font-size:16px;color:var(--navy);margin:3px 0 4px;}
.fit .lbl{font-size:9.5px;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;
  color:var(--blue);margin-top:12px;}
.fit p{font-size:13.5px;margin:3px 0 0;}
.fit .green{background:var(--cream);border-left:3px solid var(--gold);padding:9px 11px;
  margin-top:11px;font-size:13.5px;font-weight:700;color:var(--navy);}
.fit .green em{display:block;font-weight:400;font-size:11px;color:#666;margin-top:3px;}
.notbuilt{display:inline-block;background:var(--gold);color:var(--navy);font-size:9.5px;
  font-weight:700;letter-spacing:1.4px;text-transform:uppercase;padding:2px 8px;margin-bottom:8px;}
.blank{border-bottom:2px solid var(--red);padding:0 20px;}
/* asset scale band */
...
PAGES["egon-baltimore.html"] = ("EGON &mdash; Baltimore Harbor | Ripley Decision Advantage",
  "Where EGON fits in the USACE delivery process: Baltimore Harbor Navigation Project.",
  f"""
<h1>EGON</h1>
<div class="h1-rule"></div>
<h4 style="color:var(--blue);letter-spacing:4px;">Where it fits in your process</h4>

<p><strong>Baltimore Harbor Navigation Project &middot; USACE Baltimore District, North Atlantic
Division.</strong></p>

<p>EGON does not replace a step and does not ask you to change one. It sits underneath the process
you already run, and it is download-only &mdash; nothing touches HEC-RAS, HEC-FDA or ADCIRC. Your
certified models stay the system of record. EGON reads what they export.</p>

<div class="phase-bar">
  <div class="ph done"><span class="pn">1</span>Study</div>
  <div class="ph done"><span class="pn">2</span>Feasibility</div>
  <div class="ph done"><span class="pn">3</span>Alternatives</div>
  <div class="ph done"><span class="pn">4</span>Design</div>
  <div class="ph now"><span class="pn">5</span>Permitting</div>
  <div class="ph"><span class="pn">6</span>Funding</div>
  <div class="ph"><span class="pn">7</span>Construction</div>
</div>
<div class="gates">
  <div class="gate">Engineering &mdash; phases 1&ndash;4, into 5</div>
  <div class="gate">Funding &mdash; phase 6</div>
  <div class="gate">Stakeholder engagement &mdash; phase 7</div>
</div>

<div class="callout">
  <div class="ctitle">What is settled, and what is not</div>
  <p>The cost side is priced and cited. Benefit-cost ratio localisation and benefit transfer are
  <strong>unresolved</strong> &mdash; that is proof-of-concept work, not a claim. Permitting support
  is designed but <strong>not built</strong>. Everything below says which it is.</p>
</div>

<div class="fit-grid">

  <div class="fit">
    <span class="phn">02</span><div class="fh">Feasibility</div>
    <div class="lbl">The problem</div>
    <p>Floods surprise you when the analysis silently did not cover what you thought it did. A datum
    from 1992 measured against ground from 2024. A dominant asset class excluded because no curve
    exists for it. Depth measured from the dirt, fed into a curve indexed to the first floor.</p>
    <div class="lbl">What EGON does</div>
    <p>Verified open-source and government data under enforced contracts &mdash; sources named
    before anything is computed. Escrowed refresh: every pull time-stamped, staleness gated red or
    green, no human in the loop. Models draft and discover; a deterministic core is the only
    writer. Each of those gaps is written on the figure, so it surfaces in review rather than after
    the water.</p>
    <div class="green">A feasibility cycle <span class="blank">&nbsp;</span>% shorter.
    <em>Blank until your baseline</em></div>
  </div>

  <div class="fit">
    <span class="phn">03</span><div class="fh">Alternatives</div>
    <div class="lbl">The problem</div>
    <p>Alternatives are built by hand, a few at a time, by planners working in separate
    regions.</p>
    <div class="lbl">What EGON does</div>
    <p>A scenario generator surfaces the site&rsquo;s full measure catalogue &mdash; nature-based,
    gray and hybrid &mdash; rather than the handful someone had time to think of. Every measure is
    screened to approved sources, then ranked on risk-adjusted return. A rejection log records what
    was screened out and why. Both horizons are reported: the near-term hold and the long-term
    service life.</p>
    <div class="green"><span class="blank">&nbsp;</span>% more of the right alternatives, earlier.
    <em>Blank until your baseline</em></div>
  </div>

  <div class="fit">
    <span class="phn">04</span><div class="fh">Design</div>
    <div class="lbl">The problem</div>
    <p>Design maturity is whatever it has reached at the moment money is asked for, and it strains
    toward 35%.</p>
    <div class="lbl">What EGON does</div>
    <p>A USACE Class-5 cost engine built to your methods, not a generic model, alongside
    expected-annual-damage and life-cycle benefit engines running both horizons. CWCCIS escalation,
    so cross-year figures are escalated rather than summed raw. The validation gate fails closed on
    provenance and schema.</p>
    <div class="green">Design integrity <span class="blank">&nbsp;</span>% &rarr; 35% at
    appropriation. <em>Your own figures</em></div>
  </div>

  <div class="fit pending">
    <span class="phn">05</span><div class="fh">Permitting</div>
    <span class="notbuilt">Pending POC &mdash; not yet built</span>
    <div class="lbl">The problem</div>
    <p>Agency packages are assembled by hand.</p>
    <div class="lbl">What EGON could do</div>
    <p>The same deterministic core generates the agency-facing package &mdash; Section 404 and
    others &mdash; every figure carrying its source and its calculation into the submission. Same
    payload, different reader. EGON does not shorten a statutory clock. It shortens the time you
    spend assembling what goes into it.</p>
    <div class="green">Packages that arrive complete and sourced.</div>
  </div>

</div>

<h2>{icon("shield")}How the numbers stay defensible</h2>
<ul>
  <li><strong>No model in the trust path.</strong> Language models draft and discover; a
  deterministic core is the only writer, so the same run produces the same report.</li>
  <li><strong>No contact with your certified models.</strong> Download-only. Nothing interfaces
  with HEC-RAS, HEC-FDA or ADCIRC.</li>
  <li><strong>Decision support, not decision making.</strong> You export, you integrate, you
  decide.</li>
  <li><strong>The gate fails closed.</strong> A figure without a source and a reproducible
  calculation cannot be represented at all. That is a structural property, not a promise.</li>
</ul>

<h2>{icon("glasses")}What the guarantee actually is</h2>
<p>Not that every figure is good &mdash; that you know what every figure is worth. Each one arrives
with its source named, its calculation shown, its model named and carried under an alternate so you
can see what the model choice is worth &mdash; or flagged as a gap, never quietly filled in.</p>

<div class="callout">
  <div class="ctitle">A worked search, including the part that failed</div>
  <p>Pricing a coastal industrial asset at the Port of Mobile needed a depth-damage curve carrying
  wave and erosion, not inundation alone. No published USACE curve covers that case. Six sources
  were screened; none closed it. The escrowed NACCS report was read in full &mdash; all
  <span class="num">306</span> pages &mdash; and carries no coastal industrial curve. No successor
  has been issued. The riverine substitutes were rejected on the record, because they carry no wave
  or erosion mechanism.</p>
  <p>EGON reported that no benefit-cost ratio was computable, rather than producing one. It does not
  close the gap. It names it, shows the search, and stops there.</p>
</div>

<div class="callout">
  <div class="ctitle">Referee, not advocate</div>
  <p>EGON prices the exposure and lets the return choose the lever. Where a nature-based move beats
  hard infrastructure per dollar, EGON says so. Where it doesn&rsquo;t, EGON stays quiet. Poplar
  Island is the Baltimore District precedent for what beneficial-use placement can carry.</p>
</div>
...
<ul>
  ...
  over.</li>
  <li><strong>Confidential compute where data cannot travel, on the roadmap.</strong> Analysis
  against data that never leaves its owner&rsquo;s control, for the cases where sharing the
  underlying file is not on the table. Designed, not yet running.</li>
