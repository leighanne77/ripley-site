...
- **EGON status (audited against the repository, 2026-09-24)** — piloted with infrastructure customers in four countries (past, plural); four worked US sites; the first full field study, one customer, concludes late 2026. Keep the three lines separate so the track record is never read as EGON's current status.
- **Roadmap is not built** — compliance footprint, forecast triggers, monitoring and optimization, role views, confidential compute. The EGON page opens the list with "None of the following runs today"; the service page labels confidential compute as roadmap. Never present any of these as shipped.
- **Mitigation scope** — EGON tests "the mitigations in scope", never "every available mitigation", and the shortlist is one "you choose from", never "prescriptive". State what was tested and what was not.
- **Data claims** — refresh cadences are in days (90 to 1825), so no "hourly refresh"; the impressive part is the version-controlled refresh registry. "45 nature-based solutions with published citations" is one dataset, not "45+ datasets". Nothing computes statistical significance, and the payload API is not built: say neither.
- **Repository counts** — 39 ADRs (was 35 on the infographic); "20+ data loaders" is 14 factory adapters plus 37 agent-side data modules.
